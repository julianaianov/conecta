...
/**
 * Painel-vitrine das telas de auth (login/cadastro) — inspirado no login
 * moderno do Facebook, adaptado ao DM Conecta: fundo claro, título grande
 * empilhado e uma colagem de cards flutuantes (estilo adesivo) montada com
 * fotos e ícones reais do sistema. Só desktop (lg+); no mobile o formulário
 * ocupa a tela toda. Manual de Marca: Petróleo #1B4F72 · Laranja #F4841A.
 */
export function AuthShowcase() {
  return (
    <div className="relative hidden w-1/2 flex-col justify-between overflow-hidden p-12 lg:flex xl:p-16" style={{ background: "var(--th-bg)" }}>
      ...
      {/* Título (acima da colagem no z-index → sempre legível) */}
      ...
      {/* Colagem flutuante (atrás do título, canto direito) */}
      <Collage />
...
/** Composição de cards flutuantes tipo adesivo. */
function Collage() {
  return (
    <div className="pointer-events-none absolute right-6 top-1/2 z-10 hidden h-[30rem] w-[23rem] -translate-y-1/2 xl:block 2xl:right-16">
      {/* Card de trás — horta comunitária (levemente girado) */}
      <figure
      ...
      {/* Card principal — "story" de mutirão/voluntários */}
      <figure
      ...
      >
        ...
        <span className="absolute right-3 top-7 inline-flex items-center gap-1 rounded-full bg-white/95 px-2.5 py-1 text-[11px] font-bold shadow-sm" style={{ color: "#1b4f72" }}>
          <Icon name="location" size={12} style={{ color: "#f4841a" }} /> Recreio
        </span>
        {/* legenda inferior */}
      ...
      {/* Card da frente — feira/comércio local */}
      <figure
      ...
      {/* Adesivo flutuante — reação (voluntariado) */}
      ...
      {/* Adesivo flutuante — coração/apoio */}
      ...
      {/* Avatar circular sobreposto na base */}
